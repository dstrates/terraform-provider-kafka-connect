...
func rebalanceWait() time.Duration {
	if v := getenvDuration("TF_KAFKA_CONNECT_REBALANCE_WAIT_SECS"); v > 0 {
		return v
	}
...
func withRebalanceRetry(fn func() error) error {
	deadline := time.Now().Add(rebalanceWait())
	...
	for {
		...
		log.Printf("[INFO] Connect rebalance in progress; retrying after %s ... (%v)", sleep, err)
		time.Sleep(sleep)
